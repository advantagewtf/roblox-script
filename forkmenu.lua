--[[
	...
	Moonlight Interface Suite
	by Sirius moddified by ellii
]]

if debugX then
	warn('Initialising Moonlight')
end
...
local requestsDisabled = true --getgenv and getgenv().DISABLE_Moonlight_REQUESTS
local InterfaceBuild = '0'
local Release = "v1.0.0"
local MoonlightFolder = "Moonlight"
local ConfigurationFolder = MoonlightFolder.."/Configurations"
local ConfigurationExtension = ".mlui"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		MoonlightOpen = {Type = 'bind', Value = 'K', Name = 'Moonlight Keybind'},
		-- buildwarnings
		-- Moonlightprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.MoonlightOpen"] = "J"
...
local prompt = loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/Sirius/refs/heads/request/prompt.lua')
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(MoonlightFolder) then
				if isfile and isfile(MoonlightFolder..'/settings'..ConfigurationExtension) then
					file = readfile(MoonlightFolder..'/settings'..ConfigurationExtension)
				end
			end
	...
	if not success then 
		if writefile then
			warn('Moonlight had an issue accessing configuration saving capability.')
		end
	end
end
...
local Moonlight =  game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = true
local warned
local globalLoaded
local MoonlightDestroyed = false -- True when MoonlightLibrary:Destroy() is called

Moonlight.Enabled = false

if gethui then
	Moonlight.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(Moonlight)
	Moonlight.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	Moonlight.Parent = CoreGui:FindFirstChild("RobloxGui")
else 
	Moonlight.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Moonlight.Name and Interface ~= Moonlight then
			Interface.Enabled = false
			Interface.Name = "Moonlight-Old"
		end
	end
else
...
if Moonlight.AbsoluteSize.X < minSize.X and Moonlight.AbsoluteSize.Y < minSize.Y then
...
local Main = Moonlight.Main
local MPrompt = Moonlight:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Moonlight:FindFirstChild('Drag')
...
local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = MoonlightLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Moonlight.Main.BackgroundColor3 = SelectedTheme.Background
	Moonlight.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Moonlight.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Moonlight.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow
	...
	for _, text in ipairs(Moonlight:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Moonlight | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Moonlight | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Moonlight had an issue decoding the configuration file, please try delete the file and reopen Moonlight.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(MoonlightLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Moonlight | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--MoonlightLibrary:Notify({Title = "Moonlight Flags", Content = "Moonlight was unable to find '"..FlagName.. "' in the save file.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(MoonlightLibrary.Flags) do
...
function MoonlightLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			MoonlightLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			MoonlightLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "MoonlightOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(MoonlightFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Moonlight Settings', 0, true)

	if TabList['Moonlight Settings'] then
		TabList['Moonlight Settings'].LayoutOrder = 1000
	end

	if Elements['Moonlight Settings'] then
		Elements['Moonlight Settings'].LayoutOrder = 1000
	end
...
function MoonlightLibrary:CreateWindow(Settings)
	print('creating window')
	if Moonlight:FindFirstChild('Loading') then
		if getgenv and not getgenv().MoonlightCached then
			Moonlight.Enabled = true
			Moonlight.Loading.Visible = true

			task.wait(1.4)
			Moonlight.Loading.Visible = false
		end
	end

	if getgenv then getgenv().MoonlightCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				MoonlightLibrary:Notify({Title = 'Build Mismatch', Content = 'Moonlight may encounter issues as you are running an incompatible interface version ('.. ((Moonlight:FindFirstChild('Build') and Moonlight.Build.Value) or 'No Build') ..').\n\nThis version of Moonlight is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if Settings.ToggleUIKeybind then -- Can either be a string or an Enum.KeyCode
		...
		if type(keybind) == "string" then
			...
			overrideSetting("General", "MoonlightOpen", keybind)
		elseif typeof(keybind) == "EnumItem" then
			assert(keybind.EnumType == Enum.KeyCode, "ToggleUIKeybind must be a KeyCode enum")
			overrideSetting("General", "MoonlightOpen", keybind.Name)
		else
			error("ToggleUIKeybind must be a string or KeyCode enum")
		end
	end

	if isfolder and not isfolder(MoonlightFolder) then
		makefolder(MoonlightFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Moonlight"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Moonlight Interface Suite" then
		LoadingFrame.Version.Text = "Moonlight UI"
	end
	...
	if not Settings.DisableMoonlightPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				MoonlightLibrary:Notify({
					Title = "moonlight interface",
					Content = "thanks for using moonlight :)",
					Duration = 7,
					Image = 4370033185,
				})
			end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(MoonlightFolder.."/Key System") then
			makefolder(MoonlightFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Moonlight | "..Key.." Error " ..tostring(Response))
					warn('Check REMOVED for help with Moonlight specific development.')
				end
			end
		end
		...
		if isfile and isfile(MoonlightFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(MoonlightFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(MoonlightFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						MoonlightLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called MoonlightLibrary:Destroy()
				if MoonlightDestroyed then
					return
				end
				if not Success then
					...
					print("Moonlight | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check REMOVED for help with Moonlight specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= MoonlightLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					MoonlightLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Moonlight.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Moonlight | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check REMOVED for help with Moonlight specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					MoonlightLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Moonlight | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check REMOVED for help with Moonlight specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					MoonlightLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Moonlight | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check REMOVED for help with Moonlight specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					MoonlightLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= MoonlightLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Moonlight | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check REMOVED for help with Moonlight specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						MoonlightLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Moonlight | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check REMOVED for help with Moonlight specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					MoonlightLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			MoonlightLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			MoonlightLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Moonlight had an issue creating settings.') end
...
function MoonlightLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function MoonlightLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function MoonlightLibrary:Destroy()
	MoonlightDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Moonlight Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "MoonlightOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function MoonlightLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				MoonlightLibrary:Notify({Title = "Moonlight Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			MoonlightLibrary:Notify({Title = "Moonlight Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Moonlight Configurations Error | '..tostring(result))
			MoonlightLibrary:Notify({Title = "Moonlight Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
task.delay(4, function()
	MoonlightLibrary.LoadConfiguration()
